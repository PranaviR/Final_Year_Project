...
logger.info("Trial4 Hurricane Matthew BERT cos(A,B)>cos(A,C)")
logger.info("\n")
...
trainFeatures = bert(trainingFile, document_embeddings)
validationFeatures = bert(validationFile, document_embeddings)
...
initial_F1 = lr(trainFeatures, trainLabels, validationFeatures, validationLabels) 
print(initial_F1)
f1 = initial_F1
logger.info(str(f1))
...
while f1 < thresholdF1 and numberOfIterations < maxNoOfIterations:
    ...
    for tweetFeature in currentIterFeat:
        ...
        if poscosineSim > negcosineSim:
            ...
            logger.info(str(0) + ' ' + str(poscosineSim) + ' ' + str(negcosineSim) + ' ' + currentIterTweets[count])
            # print('no')
            # print(iterationLabels)
        count+=1
    ...
    print(len(iterationTrainingFeatures))
    print(len(iterationTrainingLabels))
    iterationF1 = lr(iterationTrainingFeatures, iterationTrainingLabels, validationFeatures, validationLabels) 
